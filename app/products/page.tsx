 ...
 * =====================
 * Main products page with filtering, sorting, and pagination
 * 
 * DEVELOPER NOTES:
 * - Filters are managed via URL search params for shareability
 * - Mobile filters use a slide-out sheet
 * - Products are loaded from lib/data/products.ts (can be replaced with API)
 */
...
export default function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (currentCategories.length > 0) {
      result = result.filter(p => currentCategories.includes(p.category));
    }
    ...
    return result;
  }, [allProducts, currentCategories, currentBrands, currentMinPrice, currentMaxPrice, sortBy]);

  const hasActiveFilters = currentCategories.length > 0 || currentBrands.length > 0 || currentMinPrice || currentMaxPrice;
                ...
                >
                  ...
                  {hasActiveFilters && (
                    <Badge variant="brand" className="ml-2">
                      {currentCategories.length + currentBrands.length + (currentMinPrice || currentMaxPrice ? 1 : 0)}
                    </Badge>
                  )}
                </Button>
                ...
                <div className="hidden sm:flex items-center border border-border rounded-lg p-1">
                  <button
                    className={cn(
                      ...
                      viewMode === 'grid' && 'bg-brand-green-100 text-brand-green-600'
                    )}
                  ...
                  <button
                    className={cn(
                      'p-2 rounded-md transition-colors',
                      viewMode === 'list' && 'bg-brand-green-100 text-brand-green-600'
                    )}
